...
export default class FindIntegrationTool extends Tool {
  public meta: ToolMeta = {
    ...
    function: {
      ...
      description: 'Look up the schema (fields, required fields, allowed values) of an tool on a configured integration, e.g. create_post on a wordpress site. Call this before call_integration to learn which fields to send.',
      parameters: {
        type: 'object',
        properties: {
          integration: {
  ...
  public async call(args: { integration: string, tool: string }): Promise<{ [key: string]: any }> {
    ...
    const description = info.tools[args.tool];
    if (description === undefined) {
      return {
        error: `tool "${args.tool}" does not exist on integration "${args.integration}"`,
        tool: Object.keys(info.tools),
      };
    }
    ...
    const configured = integration.config?.tools?.[args.tool]?.fields as { [key: string]: { required?: boolean, type?: string, description?: string, enum?: string[] } } | undefined;
    let fields: Field[] = [];
