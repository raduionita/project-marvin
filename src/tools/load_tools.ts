...
export default class LoadToolsTool extends Tool {
  public meta: ToolMeta = {
    ...
    function: {
      name: 'load_tools',
      description: 'Load one or more callable tools into this chat. Pass the tool names (e.g. ["read_file", "web_search", "gloobeam__create_post", "my_mcp__my_tool"]). Available tools are listed in the "## Available Tools", "## Integrations" and "## MCPs" sections of the system prompt. Integration tools use "<integrationId>__<action>" and MCP tools use "<mcpId>__<toolName>".',
      parameters: {
        type: 'object',
        properties: {
          names: {
            type: 'array',
            items: { type: 'string' },
            description: 'Names of the tools to load (e.g. ["read_file", "web_search", "gloobeam__create_post"])',
          },
        },
        required: ['names'],
      }
