...
// Convert markdown to a Slack Block Kit rich_text block so LLM output renders
// as rich text (headings, lists, code, quotes) instead of flat mrkdwn text.
// Mirrors markdownToMrkdwn's rules; returns a single rich_text block.
export function markdownToRichTextBlocks(markdown: string): any[] {
  ...
  while (i < lines.length) {
    ...
    // fenced code block -> preformatted
    if (/^\s*```/.test(line)) {
      const code: string[] = [];
      i++;
      ...
      elements.push({ type: 'rich_text_preformatted', elements: [{ type: 'text', text: code.join('\n') }] });
      continue;
    }
    ...
    if (header) {
      elements.push({ type: 'rich_text_section', elements: formatInlineRich(header[1]!, { bold: true }) });
      i++;
      continue;
    }

    // quote -> rich_text_quote (group consecutive quote lines)
    if (/^\s*>\s?/.test(line)) {
      const quote: string[] = [];
      ...
      elements.push({ type: 'rich_text_quote', elements: formatInlineRich(quote.join('\n')) });
      continue;
    }

    // list -> rich_text_list (group consecutive items of the same style)
    const listMatch = line.match(/^\s*([-*+]|\d+\.)\s+(.*)$/);
    if (listMatch) {
      const style = /^\d+\.$/.test(listMatch[1]!) ? 'ordered' : 'bullet';
      const items: any[] = [];
      while (i < lines.length) {
        const m = lines[i]!.match(/^\s*([-*+]|\d+\.)\s+(.*)$/);
        if (!m) break;
        const itemStyle = /^\d+\.$/.test(m[1]!) ? 'ordered' : 'bullet';
        if (itemStyle !== style) break;
        items.push({ type: 'rich_text_section', elements: formatInlineRich(m[2]!) });
        i++;
      }
      elements.push({ type: 'rich_text_list', style, elements: items });
      continue;
    }
    ...
    // plain paragraph
    elements.push({ type: 'rich_text_section', elements: formatInlineRich(line) });
    i++;
  }
...
// inline markdown -> rich text elements: bold, italic, strike, code, links
function formatInlineRich(text: string, baseStyle?: { bold?: boolean }): any[] {
  const out: any[] = [];
  const re = /(\*\*[^*]+\*\*|\*[^*\n]+\*|~~[^~]+~~|`[^`\n]+`|\[[^\]]+\]\([^)\s]+\))/g;
  let last = 0;
  ...
  while ((m = re.exec(text)) !== null) {
    if (m.index > last) {
      out.push({ type: 'text', text: text.slice(last, m.index), ...(baseStyle ? { style: baseStyle } : {}) });
    }
    const token = m[0];
    if (token.startsWith('**') && token.endsWith('**')) {
      out.push({ type: 'text', text: token.slice(2, -2), style: { ...baseStyle, bold: true } });
    } else if (token.startsWith('*') && token.endsWith('*')) {
      out.push({ type: 'text', text: token.slice(1, -1), style: { ...baseStyle, italic: true } });
    } else if (token.startsWith('~~') && token.endsWith('~~')) {
      out.push({ type: 'text', text: token.slice(2, -2), style: { ...baseStyle, strike: true } });
    } else if (token.startsWith('`') && token.endsWith('`')) {
      out.push({ type: 'text', text: token.slice(1, -1), style: { ...baseStyle, code: true } });
    } else {
      const link = token.match(/^\[([^\]]+)\]\(([^)\s]+)\)$/);
      if (link) {
        out.push({ type: 'text', text: link[1]!, link: link[2]!, ...(baseStyle ? { style: baseStyle } : {}) });
      } else {
  ...
  if (last < text.length) {
    out.push({ type: 'text', text: text.slice(last), ...(baseStyle ? { style: baseStyle } : {}) });
  }
  return out;
}
