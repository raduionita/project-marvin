...
export default class SkillsCommand extends Command {
  ...
  async exec() {
    console.debug('[SkillsCommand.exec]');

    const cmd = this.args[0] || 'help';
    switch (cmd) {
      default:
        console.warn('[SkillsCommand.exec]', 'unknown command: skills', cmd);
      case 'help':
        await this.execHelp();
      break;
  ...
  // `marvin skills help`
  async execHelp() {
    console.info('usage: marvin skills [command] [--dry]');
    console.info('commands:');
    console.info('  help                 ', 'show this help');
    console.info('  list                 ', 'list available skills');
    console.info('  add <name> [desc]    ', 'create a custom skill by prompting the LLM (~/.marvin/skills)');
    console.info('  use [skill]          ', 'use a skill: pick a skill, answer its questions, run it (tools-create/tools-edit write ~/.marvin/tools)');
  }

  // `marvin skills list`
  async execList() {
    console.debug('[SkillsCommand.execList]');
    ...
    console.info('default skills:');
    if (defaults.length === 0) console.info('  (none)');
    for (const id of defaults) {
      const skill = this.engine.skills[id];
      console.info(`  ${id}`);
      if (skill?.description) console.info('  -', skill.description);
    }

    console.info('custom skills:');
    if (custom.length === 0) console.info('  (none)');
  ...
  // `marvin skills add [name] [description]` // create a custom skill
  async execAdd() {
    console.debug('[SkillsCommand.execAdd]', 'creating a skill...');
    ...
    if (!name || !/^[a-zA-Z0-9_-]+$/.test(name)) {
      console.error('[SkillsCommand.execAdd]', 'invalid skill name (use a-z, 0-9, _ and -):', name);
      return;
    }
    ...
    if (!description) {
      console.error('[SkillsCommand.execAdd]', 'no description provided, exiting');
      return;
    }
    ...
    if (existsSync(spath)) {
      console.warn(`skill "${name}" already exists at ${spath}`);
      return;
    }
    ...
    if (!meta) {
      console.error('[SkillsCommand.execAdd]', 'the "meta" skill is not loaded, cannot create skills');
      return;
    }
    ...
    if (this.engine.isDry) {
      ...
      if (!result || !result.content) {
        console.error('[SkillsCommand.execAdd]', 'no result from the LLM');
        return;
      }
    ...
    if (this.engine.isDry) {
      console.info('[dry]', `would write skill to ${spath}`);
    } else {
    ...
    console.info(`skill "${id}" created, saved to ${spath}`);
  }

  // `marvin skills use [skill] [<name>]` // apply a skill interactively
  async execUse() {
    console.debug('[SkillsCommand.execUse]', 'using a skill...');
    ...
    if (!id) {
      console.info('available skills:');
      for (const sid of ids) {
        const skill = this.engine.skills[sid];
        console.info(`  ${sid}`, skill?.description ? '- ' + skill.description : '');
      }
      id = (await ask('Pick a skill: ')).toLowerCase();
      if (!id) {
        console.error('[SkillsCommand.execUse]', 'no skill selected, exiting');
        return;
      }
    }
    ...
    const skill = this.engine.skills[id];
    if (!skill) {
      console.error('[SkillsCommand.execUse]', 'unknown skill:', id);
      return;
    }
    ...
    if (isToolCreate || isToolEdit) {
      ...
      if (!toolName) {
        console.error('[SkillsCommand.execUse]', 'invalid tool name (use a-z, 0-9, _):', toolName);
        return;
      }
      if (isToolCreate) {
        ...
        if (!existsSync(tpath)) {
          console.error('[SkillsCommand.execUse]', 'tool does not exist in ~/.marvin/tools:', toolName);
          return;
        }
        info = await ask('What should change about the tool? ');
      }
      if (!info) {
        console.error('[SkillsCommand.execUse]', 'no tool description provided, exiting');
        return;
      }
    } else {
      info = await ask('Describe what you want to do with this skill: ');
      if (!info) {
        console.error('[SkillsCommand.execUse]', 'no task provided, exiting');
        return;
      }
    }
    ...
    if (this.engine.isDry) {
      ...
      const result = await this.engine.execChat(undefined, this.engine.config.settings.name, prompt, 'text');
      if (!result || !result.content) {
        console.error('[SkillsCommand.execUse]', 'no result from the LLM');
        return;
      }
    ...
    if (isToolCreate || isToolEdit) {
      if (this.engine.isDry) {
        console.info('[dry]', isToolCreate ? 'would create tool in ~/.marvin/tools' : 'would update tool in ~/.marvin/tools');
      } else {
        ...
        console.info(`${isToolCreate ? 'tool' : 'tool'} "${toolName.toLowerCase()}" ${isToolCreate ? 'created' : 'updated'}, saved to ${tpath}`);
      }
    }

    console.info('');
    console.info(`used skill "${id}":`);
    console.info(output || '(dry run, no output produced)');
  }
}
