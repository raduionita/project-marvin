...
export default class UpdateCommand extends Command {
  async init() {
    ...
    if (!existsSync(root)) {
      console.error('marvin is not installed. run the installer first:');
    ...
    if (this.ctx.isDry) {
      console.log('[dry] pull project:', ['git', '-C', root, 'pull', 'origin', 'main'].join(' '));
      console.log('[dry] install dependencies:', ['bun', 'install'].join(' '));
      console.log('[dry] restart service:', ['systemctl', '--user', 'restart', 'marvin'].join(' '));
    } else {
      // git pull from main
      console.info('pulling project...');
      ...
      console.info('updating service file...');
